...
statement = 'create table if not exists FEEDBACK(lot_no string, scores json, ' + 'primary key(lot_no)'
req = TableRequest().set_statement(statement).set_table_limits(TableLimits(20,10,5))
result = handle.do_table_request(req,40000,3000)
...
def soil_fertility(fi, fs):#fertility index missing in frontend
...
@app.route("/")
def home():
    statement = 'select * from SEED_BLOCK'
    req = QueryRequest().set_statement(statement)
    results = []
    while True:
        result = handle.query(req).get_results()
    ...
    if "username" not in session:
        return render_template("index.html",login_type="",num=num)
    ...
    if type_login == "farmer":
        ...
        session["name"]=name
        return render_template("index.html",login_type=agencyType,num=num)
...
def spp_update():
    ...
    if request.method == "POST":
        ...
        fs = data["fertilityStatus"]
        fi = data["fertilityIndex"]
        fertility_status , trust = soil_fertility(fi,fs)
        
        if fertility_status !=-1:
            data["calculatedFertilityStatus"]=fertility_status
            data["trust"]=trust
...
def feedback():
    ...
    if request.method=="POST":
        ...
        dict1 = json.loads(dict(results[0])["seed_data"])
        dname = dict1["distributorName"]
        certified= dict1["samplePassed"]
        stlName = dict1["stlName"]
        sgName=dict1["sgName"]

        scores = scoring(score,dname,certified,stlName,sgName)
        
        req = PutRequest().set_table_name('FEEDBACK')
        req.set_value({'lot_no': lno,'scores':json.dumps(scores)})
        result = handle.put(req)

        statement = 'select scores from FEEDBACK'
        req = QueryRequest().set_statement(statement)
        results = []
        while True:
            result = handle.query(req).get_results()
        ...
        for i in range(0,len(results)):
            temp = json.loads(dict(results[i])["scores"])
            stl = stl + temp["stl"]
            dis = dis + temp["dis"]
            sg = sg + temp["sg"]
            sentiment = sentiment + temp["sentiment"]
        ...
        return render_template("feedback.html",scores=json.dumps({"stl":stl,"dis":dis,"sg":sg,"sentiment":sentiment}))
